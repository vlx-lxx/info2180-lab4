...
if (!empty($searchTerm)) {
    // Search for the superheroes in database
    foreach ($superheroes as $superhero) {
        if (
            strpos(strtolower($superhero['name']), $searchTerm) !== false ||
            strpos(strtolower($superhero['alias']), $searchTerm) !== false
        ) {

            // Superhero found, display their information
    ...
    // Checking if superheroes are in database.
    if (!$heroesFound) {
        echo "<h5>SUPERHERO NOT FOUND</h5>";
    }
} else {
    // In the event no search item show full list
    echo "<ul>";
    foreach ($superheroes as $superhero) {
